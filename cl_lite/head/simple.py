...
class SimpleHead(nn.Module):
    ...
    def _create_classifier(
        ...
        classifier = nn.Sequential(nn.Linear(num_features, num_features, bias=bias),
                                   nn.Linear(num_features, num_classes, bias=bias)) #self.lsoftmax_linear = LSoftmaxLinear(
            #input_features=num_features, output_features=num_classes, margin=margin, device=self.device)
