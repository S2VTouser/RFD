...
from cl_lite.nn import freeze, unfreeze, RKDAngleLoss, DIST
...
class Module(FeatureHookMixin, FinetuningMixin, cl.Module):
    ...
    def __init__(
            ...
            lambda_distkd_old: float = 0.5,
            lambda_distkd_new: float = 0.5,
            lambda_kd: float = 0.5,
    ...
    def register_losses(self):
        ...
        self.register_loss(
            "distkd_old",
            DIST(),
            ["input_distkd_old", "target_distkd_old"],
            self.hparams.lambda_distkd_old * alpha * beta2,
        )
        self.register_loss(
            "distkd_new",
            DIST(),
            ["input_distkd_new", "target_distkd_new"],
            self.hparams.lambda_distkd_new * alpha * beta2,
        )
        self.register_loss(
            "sm",
            nn.MSELoss(reduction='mean'),
            ["fea_old", "fea_new"],
            self.hparams.lambda_kd * alpha * beta,
        )
    ...
    def init_setup(self, stage=None):
        ...
        self.model_old, self.inversion = None, None  # , nn.Identity()
    ...
    def check_finetuning(self, at_epoch_end: bool = True):
        ...
        if self.model_old is not None:
            _ = [self.unregister_loss(name) for name in ["sm"]]
            self.register_loss(
                "ce",
    ...
    def training_step(self, batch, batch_idx): 
        ...
        if self.model_old is not None:
            ...
            for i in range(input.shape[0]):
                 save_image(input_rh[i], './gen_imgs/%d.jpg'%(i))
                 save_image(input[i], './imgs/%d.jpg'%(i))
            target_all = torch.cat([target_t, target_rh])

            feature_new = self.current_feature("pen")  # save
            prediction_old = self(input_rh)
            _, features_old = self.model_old.backbone(input, flag=True)
            fea_old_s1 = features_old[0]
            fea_old_end = self.backbone.get_features(fea_old_s1)
         
            size = fea_old_end.size()
            N, C = size[:2]
            feat_mean = fea_old_end.view(N, C, -1).mean(dim=2)   #.view(N, C, 1, 1)
            fea_old_end = feat_mean 
            
            n_old = self.model_old.head.num_classes

            ## KD old
            old_inputrh = self.model_old(input_rh)
            kwargs["input_distkd_old"] = prediction_old[:, :n_old]
            kwargs["target_distkd_old"] = old_inputrh 
            ## KD new
            old_input = self.model_old(input)
            kwargs["input_distkd_new"] = kwargs["prediction"][:, :n_old]
            kwargs["target_distkd_new"] = old_input

            if not self.finetuning:
                kwargs["fea_new"], kwargs["fea_old"] = feature_new, fea_old_end
    ...
    def configure_optimizers(self):
        module = nn.ModuleList([self.backbone, self.head])
        optimizer = optim.SGD(
            module.parameters(),
